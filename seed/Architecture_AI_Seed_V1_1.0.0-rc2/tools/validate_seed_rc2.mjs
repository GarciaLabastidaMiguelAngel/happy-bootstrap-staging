...
for(const token of ['1.0.0-rc2','SEED-SNAPSHOT-RC2-001','P-SEED-RC2-GIT-PUBLISH-01','HAPPY_HANDOFF_READY = TRUE','SPECIFICATION_BASELINE_READY_FOR_BUILD = FALSE'])if(!manifest.includes(token))errors.push(`MANIFEST_IDENTITY_MISMATCH ${token}`);
...
assertUniqueDefinitions('knowledge/62_RESEARCH_OBLIGATIONS_STANDARDS_AND_GAPS.md',/^\| `(RO-(?:3C|3E|ACCEPT|RC2)-\d{3})` \|/gm,'RO_ID');
...
const rc2=fs.readFileSync(path.join(root,'knowledge/90_POST_RC1_RECONCILIATION.md'),'utf8');
for(const token of ['SRC-RC2-001','SRC-RC2-002','SRC-RC2-003','FX-RC2-A','FX-RC2-B','FX-RC2-C','FX-RC2-D','FX-RC2-E','FX-RC2-F','RO-RC2-001','EXP-RC2-001','EXP-RC2-002','KNOWN_BUT_NOT_ACCESSIBLE'])if(!rc2.includes(token))errors.push('RC2_MISSING '+token);
for(const f of ['README.md','BOOTSTRAP.md','CONTEXT_PACK.md','REPOSITORY_MAP.md','EXPANSION_OBLIGATIONS.md','ACCEPTANCE.md'])if(!fs.readFileSync(path.join(root,f),'utf8').includes('90_POST_RC1_RECONCILIATION.md'))errors.push('RC2_NAVIGATION '+f);
assertUniqueDefinitions('knowledge/82_DEVIN_EXPANSION_OBLIGATIONS.md',/^\| `(EXP-(?:3E|ACCEPT|RC2)-\d{3})` \|/gm,'EXP_ID');
const roCount=[...fs.readFileSync(path.join(root,'knowledge/62_RESEARCH_OBLIGATIONS_STANDARDS_AND_GAPS.md'),'utf8').matchAll(/^\| `(RO-(?:3C|3E|ACCEPT|RC2)-\d{3})` \|/gm)].length;
if(roCount!==26)errors.push('RO_COUNT '+roCount);
if(specFiles.length!==10)errors.push('SPEC_COUNT_CHANGED '+specFiles.length);
const result={
