...
Current rc2 supplement: load [post-rc1 reconciliation](knowledge/90_POST_RC1_RECONCILIATION.md) with the manifest before deriving work. Projection set consistency, Graph modeling research and diagnostic obligations are targets, not implementation evidence. Reconcile the latest authorized Java directive before treating historical baseline text as current. No new numbered Devin prompt is required.
